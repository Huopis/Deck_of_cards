...
void Card::display(){
    std::cout << "Card: " << power_to_string.find(power_)->second << " of " << suite_to_string.find(suite_)->second << "\n"; 
}
...
Deck::Deck(){
    ...
    std::shuffle(std::begin(cards_buffer_), std::end(cards_buffer_), random_number_generator_);
}
